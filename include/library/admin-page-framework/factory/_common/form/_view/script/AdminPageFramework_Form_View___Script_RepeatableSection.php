...
class WPFavoritePostsAdminPageFramework_Form_View___Script_RepeatableSection extends WPFavoritePostsAdminPageFramework_Form_View___Script_Base {
...
( function( $ ) {
     ...
     */
    $.fn.updateWPFavoritePostsAdminPageFrameworkRepeatableSections = function( aSettings ) {
    
        var _oThis                = this;
        var _sSectionsContainerID = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.wp-favorite-posts-sections' ).attr( 'id' );
    
        // Store the sections specific options in an array.
        if ( ! $.fn.aWPFavoritePostsAdminPageFrameworkRepeatableSectionsOptions ) {
            $.fn.aWPFavoritePostsAdminPageFrameworkRepeatableSectionsOptions = [];
        }
        if ( ! $.fn.aWPFavoritePostsAdminPageFrameworkRepeatableSectionsOptions.hasOwnProperty( _sSectionsContainerID ) ) {
            $.fn.aWPFavoritePostsAdminPageFrameworkRepeatableSectionsOptions[ _sSectionsContainerID ] = $.extend(
                {    
        ...
        var _aOptions = $.fn.aWPFavoritePostsAdminPageFrameworkRepeatableSectionsOptions[ _sSectionsContainerID ];
        ...
        $( _oThis ).find( '.repeatable-section-add-button' ).click( function() {
            ...
            $( this ).addWPFavoritePostsAdminPageFrameworkRepeatableSection();
        ...
        $( _oThis ).find( '.repeatable-section-remove-button' ).click( function() {
            $( this ).removeWPFavoritePostsAdminPageFrameworkRepeatableSection();
            return false; // will not click after that
        });     
        
        // If the number of sections is less than the set minimum value, add sections. 
        var _sSectionID           = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.wp-favorite-posts-section' ).attr( 'id' );
        var _nCurrentSectionCount = jQuery( '#' + _sSectionsContainerID ).find( '.wp-favorite-posts-section' ).length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentSectionCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentSectionCount ) > 0 ) {     
                $( '#' + _sSectionID ).addWPFavoritePostsAdminPageFrameworkRepeatableSection( _sSectionID );  
            }
        }
     ...
     */    
    $.fn.addWPFavoritePostsAdminPageFrameworkRepeatableSection = function( sSectionContainerID ) {
        
        // Local variables
        if ( 'undefined' === typeof sSectionContainerID ) {
            var sSectionContainerID = $( this ).closest( '.wp-favorite-posts-section' ).attr( 'id' );
        }
        var nodeSectionContainer    = $( '#' + sSectionContainerID );
        var nodeNewSection          = nodeSectionContainer.clone(); // clone without bind events.
        var nodeSectionsContainer   = nodeSectionContainer.closest( '.wp-favorite-posts-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );        
        var nodeTabsContainer       = $( this ).closest( '.wp-favorite-posts-section-tabs-contents' )
            .children( '.wp-favorite-posts-section-tabs' )
            .first();
            
        var _iSectionIndex          = nodeSectionsContainer.attr( 'data-largest_index' );
        
        var _iFadein                = $.fn.aWPFavoritePostsAdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'fadein' ];
        var _iFadeout               = $.fn.aWPFavoritePostsAdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'fadeout' ];
        
        // If the set maximum number of sections already exists, do not add.
        var _sMaxNumberOfSections   = $.fn.aWPFavoritePostsAdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'max' ];
        if ( _sMaxNumberOfSections != 0 && nodeSectionsContainer.find( '.wp-favorite-posts-section' ).length >= _sMaxNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.wp-favorite-posts-repeatable-section-buttons' ).last();
        ...
        var _sSectionTabSlug = nodeNewSection.find( '.wp-favorite-posts-section-caption' ).first().attr( 'data-section_tab' );
        if ( ! _sSectionTabSlug || _sSectionTabSlug === '_default' ) {
            nodeNewSection.find( '.wp-favorite-posts-section-title' ).not( '.wp-favorite-posts-collapsible-section-title' ).hide();
        }
        ...
        if( 'function' === typeof nodeNewSection.enableWPFavoritePostsAdminPageFrameworkCollapsibleButton ){ 
            nodeNewSection.find( '.wp-favorite-posts-collapsible-sections-title, .wp-favorite-posts-collapsible-section-title' ).enableWPFavoritePostsAdminPageFrameworkCollapsibleButton();
        }
        ...
        $( nodeNewSection ).find( '.wp-favorite-posts-field' ).each( function( iFieldIndex ) {
            ...
            $( this ).updateWPFavoritePostsAdminPageFrameworkRepeatableFields();
                                        
            // Callback the registered callback functions.
            
            // @deprecated 3.8.8 Kept for backward compatibility.
            $( this ).trigger( 
                'wp-favorite-posts_added_repeatable_field', 
                [
            ...
            // 3.8.8 
            $( this ).trigger( 
                'wp-favorite-posts_repeated_field', 
                [
                    1, // call type, 0: repeatable fields, 1: repeatable sections, 
                    jQuery( nodeNewSection ).closest( '.wp-favorite-posts-sections' )    // model container
                ]
            );              
        ...
        nodeNewSection.updateWPFavoritePostsAdminPageFrameworkRepeatableSections();    
        
        // Rebind sortable fields - iterate sortable fields containers.
        nodeNewSection.find( '.wp-favorite-posts-fields.sortable' ).each( function() {
            $( this ).enableWPFavoritePostsAdminPageFrameworkSortableFields();
        });
        ...
        if ( nodeTabsContainer.length > 0 && ! nodeSectionContainer.hasClass( 'is_subsection_collapsible' ) ) {
            ...
            nodeTabsContainer.closest( '.wp-favorite-posts-section-tabs-contents' ).createTabs( 'refresh' );
        ...
        var _incrementAttributes = function( oElement, iSectionsCount, oSectionsContainer ) {
            ...
            $( oElement ).find( 'tr.wp-favorite-posts-fieldrow, .wp-favorite-posts-fieldset, .wp-favorite-posts-fields, .wp-favorite-posts-field, table.form-table, input,textarea,select' )
                .incrementAttribute( 
                    'id', 
                    iSectionsCount,
                    _sSectionIDModel
            ...
            $( oElement ).find( '.wp-favorite-posts-fields' ).incrementAttribute( 
                'data-field_tag_id_model',
                iSectionsCount,
                _sSectionIDModel
            );
            $( oElement ).find( '.wp-favorite-posts-fields' ).incrementAttributes( 
        ...
        // @todo this may be able to be removed
            $( oElement ).find( '.wp-favorite-posts-fieldset' ).incrementAttribute( 
                'data-field_id',
                iSectionsCount,
                _sSectionIDModel
            );
     ...
     */
    $.fn.removeWPFavoritePostsAdminPageFrameworkRepeatableSection = function() {
        
        // Local variables - preparing to remove the sections container element.
        var nodeSectionContainer    = $( this ).closest( '.wp-favorite-posts-section' );
        var sSectionContainerID     = nodeSectionContainer.attr( 'id' );
        var nodeSectionsContainer   = $( this ).closest( '.wp-favorite-posts-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );        
        var nodeTabsContainer       = $( this ).closest( '.wp-favorite-posts-section-tabs-contents' )
            .children( '.wp-favorite-posts-section-tabs' )
            .first();
        var nodeTabs                = nodeTabsContainer.children( '.wp-favorite-posts-section-tab' );
        ...
        var _sMinNumberOfSections = $.fn.aWPFavoritePostsAdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'min' ];
        if ( _sMinNumberOfSections != 0 && nodeSectionsContainer.find( '.wp-favorite-posts-section' ).length <= _sMinNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.wp-favorite-posts-repeatable-section-buttons' ).last();
        ...
        nodeSectionContainer.fadeOut( _iFadeout, function() { 
            ...
            if ( 1 === _nodeRemoveButtons.length ) {
                ...
                var _sSectionTabSlug = nodeSectionsContainer.find( '.wp-favorite-posts-section-caption' ).first().attr( 'data-section_tab' );
                if ( ! _sSectionTabSlug || '_default' === _sSectionTabSlug ) {
                    nodeSectionsContainer.find( '.wp-favorite-posts-section-title' ).first().show();
                }                
            }            
...
var _iSectionIndex = _iIterationIndex;
            ...
            // Call the registered callback functions.
            // @deprecated  3.6.0
            // $( this ).find( '.wp-favorite-posts-field' ).each( function( iFieldIndex ) {    
...
JAVASCRIPTS;
    ...
    static public function getEnabler($sContainerTagID, $iSectionCount, $asArguments, $oMsg) {
        ...
        new self($oMsg);
        $_oFormatter = new WPFavoritePostsAdminPageFramework_Form_Model___Format_RepeatableSection($asArguments, $oMsg);
        $_aArguments = $_oFormatter->get();
        $_sButtons = "<div class='wp-favorite-posts-repeatable-section-buttons-outer-container'>" . "<div " . self::___getContainerAttributes($_aArguments, $oMsg) . ' >' . "<a " . self::___getRemoveButtonAttributes($sContainerTagID, $oMsg, $iSectionCount) . ">-</a>" . "<a " . self::___getAddButtonAttributes($sContainerTagID, $oMsg, $_aArguments) . ">+</a>" . "</div>" . "</div>" . WPFavoritePostsAdminPageFramework_Form_Utility::getModalForDisabledRepeatableElement('repeatable_section_disabled_' . $sContainerTagID, $_aArguments['disabled']);
...
jQuery( document ).ready( function() {
    ...
    // Adds the buttons
    jQuery( '#{$sContainerTagID} .wp-favorite-posts-section-caption' ).each( function(){
        
        jQuery( this ).show();
        
        var _oButtons = jQuery( $_sButtonsHTML );
        if ( jQuery( this ).children( '.wp-favorite-posts-collapsible-section-title' ).children( 'fieldset' ).length > 0 ) {
            _oButtons.addClass( 'section_title_field_sibling' );
        }
        var _oCollapsibleSectionTitle = jQuery( this ).find( '.wp-favorite-posts-collapsible-section-title' );
    ...
    // Update the fields     
    jQuery( '#{$sContainerTagID}' ).updateWPFavoritePostsAdminPageFrameworkRepeatableSections( $_aJSArray ); 
});            
JAVASCRIPTS;
        return "<script type='text/javascript' class='wp-favorite-posts-section-repeatable-script'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>";
    }
    static private function ___getContainerAttributes(array $aArguments, $oMsg) {
        $_aAttriubtes = array('class' => self::getClassAttribute('wp-favorite-posts-repeatable-section-buttons', empty($aArguments['disabled']) ? '' : 'disabled'),);
