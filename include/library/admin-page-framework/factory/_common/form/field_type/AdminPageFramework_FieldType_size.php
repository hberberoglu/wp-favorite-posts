...
class WPFavoritePostsAdminPageFramework_FieldType_select extends WPFavoritePostsAdminPageFramework_FieldType {
    ...
    protected function getStyles() {
        return ".wp-favorite-posts-field-select .wp-favorite-posts-input-label-container {vertical-align: top; }.wp-favorite-posts-field-select .wp-favorite-posts-input-label-container {padding-right: 1em;}";
    }
    protected function getField($aField) {
        $_oSelectInput = new WPFavoritePostsAdminPageFramework_Input_select($aField['attributes']);
        if ($aField['is_multiple']) {
            $_oSelectInput->setAttribute(array('select', 'multiple'), 'multiple');
        }
        return $aField['before_label'] . "<div " . $this->getLabelContainerAttributes($aField, 'wp-favorite-posts-input-label-container wp-favorite-posts-select-label') . ">" . "<label for='{$aField['input_id']}'>" . $aField['before_input'] . $_oSelectInput->get($aField['label']) . $aField['after_input'] . "<div class='repeatable-field-buttons'></div>" . "</label>" . "</div>" . $aField['after_label'];
    }
}
class WPFavoritePostsAdminPageFramework_FieldType_size extends WPFavoritePostsAdminPageFramework_FieldType_select {
    ...
    protected function getStyles() {
        return ".wp-favorite-posts-field-size input {text-align: right;}.wp-favorite-posts-field-size select.size-field-select {vertical-align: 0px; }.wp-favorite-posts-field-size label {width: auto; } .form-table td fieldset .wp-favorite-posts-field-size label {display: inline-block;}";
    }
    protected function getField($aField) {
    ...
    private function _getNumberInputPart(array $aField, array $aBaseAttributes, $isKey, $bMultiLabels) {
        ...
        $_sLabel = $this->getElementByLabel($aField['label'], $isKey, $aField['label']);
        return "<label " . $this->getAttributes($_aSizeLabelAttributes) . ">" . $this->getElement($aField, $bMultiLabels ? array('before_label', $isKey, 'size') : array('before_label', 'size')) . ($aField['label'] && !$aField['repeatable'] ? "<span " . $this->getLabelContainerAttributes($aField, 'wp-favorite-posts-input-label-string') . ">" . $_sLabel . "</span>" : "") . "<input " . $this->getAttributes($_aSizeAttributes) . " />" . $this->getElement($aField, $bMultiLabels ? array('after_input', $isKey, 'size') : array('after_input', 'size')) . "</label>";
    }
    private function _getUnitSelectInput(array $aField, array $aBaseAttributes, $isKey, $bMultiLabels) {
        ...
        $_oUnitInput = new WPFavoritePostsAdminPageFramework_Input_select($_aUnitAttributes + array('select' => $_aUnitAttributes));
