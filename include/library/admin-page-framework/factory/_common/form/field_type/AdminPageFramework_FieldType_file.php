...
class WPFavoritePostsAdminPageFramework_FieldType_text extends WPFavoritePostsAdminPageFramework_FieldType {
    public $aFieldTypeSlugs = array('text', 'password', 'date', 'datetime', 'datetime-local', 'email', 'month', 'search', 'tel', 'url', 'week',);
    protected $aDefaultKeys = array();
    protected function getStyles() {
        return ".wp-favorite-posts-field.wp-favorite-posts-field-text > .wp-favorite-posts-input-label-container {vertical-align: middle; }.wp-favorite-posts-field.wp-favorite-posts-field-text > .wp-favorite-posts-input-label-container.wp-favorite-posts-field-text-multiple-labels {display: block;}";
    }
    ...
    private function _getFieldOutputByLabel($sKey, $sLabel, $aField) {
        ...
        $_sClassSelector = $_bIsArray ? 'wp-favorite-posts-field-text-multiple-labels' : '';
        ...
        $_aOutput = array($this->getElementByLabel($aField['before_label'], $sKey, $aField['label']), "<div class='wp-favorite-posts-input-label-container {$_sClassSelector}'>", "<label for='" . $_aInputAttributes['id'] . "'>", $this->getElementByLabel($aField['before_input'], $sKey, $aField['label']), $_sLabel ? "<span " . $this->getLabelContainerAttributes($aField, 'wp-favorite-posts-input-label-string') . ">" . $_sLabel . "</span>" : '', "<input " . $this->getAttributes($_aInputAttributes) . " />", $this->getElementByLabel($aField['after_input'], $sKey, $aField['label']), "</label>", "</div>", $this->getElementByLabel($aField['after_label'], $sKey, $aField['label']),);
        return implode('', $_aOutput);
    }
}
class WPFavoritePostsAdminPageFramework_FieldType_file extends WPFavoritePostsAdminPageFramework_FieldType_text {
