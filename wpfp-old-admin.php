...
if ( isset($_POST['submit']) ) {
	...
	$wpfp_options['thumbnail_default'] = htmlspecialchars($_POST['thumbnail_default']);
	$wpfp_options['thumbnail_alignment'] = htmlspecialchars($_POST['thumbnail_alignment']);
	$wpfp_options['thumbnail_width'] = htmlspecialchars($_POST['thumbnail_width']);
	$wpfp_options['thumbnail_height'] = htmlspecialchars($_POST['thumbnail_height']);

    $wpfp_options['thumbnail_show'] = '';
    if (isset($_POST['thumbnail_show']))
        $wpfp_options['thumbnail_show'] = htmlspecialchars($_POST['thumbnail_show']);
...
if ( isset($_GET['action'] ) ) {
	if ($_GET['action'] == 'reset-statistics') {
		...
		if ($wpdb->query($query)) {
			$message .= "All statistics data for WP Favorite Posts has been <strong>deleted</strong>.";
		} else {
			$message .= "Something's gone <strong>wrong</strong>. Data couldn't delete. Maybe there isn't any data to delete?";
		}	
		$message .= '</p></div>';
	}
}
...
<div class="postbox">
    ...
    <div class="inside" style="display: block;">
        <img src="<?php echo plugins_url( '', __FILE__ ) ; ?>/img/icon_coffee.png" alt="buy me a coffee" style=" margin: 5px; float:left;" />
        ...
        <form action="https://www.paypal.com/cgi-bin/webscr" method="post">
            ...
            ">
            <input type="image" src="<?php echo plugins_url( '', __FILE__ ) ; ?>/img/donate.gif" border="0" name="submit" alt="PayPal - The safer, easier way to pay online.">
        </form>
...
<div class="postbox">
    ...
    <div class="inside" style="display: block;">
        ...
        <table class="form-table">
            ...
            <tr>
                <th><?php _e("Display post thumbnails in favorites list?", "wp-favorite-posts") ?></th>
                <td><input type="checkbox" name="thumbnail_show" value="1" <?php if (stripslashes($wpfp_options['thumbnail_show']) == "1") echo "checked='checked'"; ?> /></td>
            </tr>

            <tr>
                <th><?php _e("Default thumbnail image?", "wp-favorite-posts") ?></th>
                <td>
                    <label for="thumbnail_default">
                        Default thumbnail URL (optional):
                    </label>
                    <input type="thumbnail_default" name="thumbnail_default" value="<?php echo stripslashes($wpfp_options['thumbnail_default']); ?>" />
                </td>
            </tr>

            <tr>
                <th><?php _e("Thumbnail alignment CSS class:", "wp-favorite-posts") ?></th>
                <td>
                    <select name="thumbnail_alignment">
                        <option value="" <?php if ($wpfp_options['thumbnail_alignment'] == '') echo "selected='selected'" ?>>Natural</option>
                        <option value="alignnone" <?php if ($wpfp_options['thumbnail_alignment'] == 'alignnone') echo "selected='selected'" ?>>None</option>
                        <option value="alignleft" <?php if ($wpfp_options['thumbnail_alignment'] == 'alignleft') echo "selected='selected'" ?>>Left</option>
                        <option value="aligncenter" <?php if ($wpfp_options['thumbnail_alignment'] == 'aligncenter') echo "selected='selected'" ?>>Center</option>
                        <option value="alignright" <?php if ($wpfp_options['thumbnail_alignment'] == 'alignright') echo "selected='selected'" ?>>Right</option>
                    </select>
                    Select <strong>Natural</strong> to add no CSS class. Select any other option to use WordPress alignment classes.
                </td>
            </tr>

            <tr>
                <th><?php _e("Thumbnail width?", "wp-favorite-posts") ?></th>
                <td>
                    <label for="thumbnail_width">
                        Thumbnail width:
                    </label>
                    <input type="thumbnail_width" name="thumbnail_width" value="<?php echo stripslashes($wpfp_options['thumbnail_width']); ?>" />
                </td>
            </tr>


            <tr>
                <th><?php _e("Thumbnail height?", "wp-favorite-posts") ?></th>
                <td>
                    <label for="thumbnail_height">
                        Thumbnail height:
                    </label>
                    <input type="thumbnail_height" name="thumbnail_height" value="<?php echo stripslashes($wpfp_options['thumbnail_height']); ?>" />
                </td>
            </tr>
        	...
        	<tr><td></td>
                <td>
                	<div class="submitbox">
						...
						<a href="?page=wp-favorite-posts&amp;action=reset-statistics" id="wpfp-reset-statistics" class="submitdelete deletion">Reset Statistics Data</a>
						</div>
					</div>
                </td>
            </tr>
...
<div class="postbox">
    ...
    <div class="inside" style="display: block;">
        ...
        <table class="form-table">
            ...
            <tr>
                <th><?php _e("Text for favorites are empty", "wp-favorite-posts") ?></th><td><textarea name="favorites_empty" rows="3" cols="35"><?php echo stripslashes($wpfp_options['favorites_empty']); ?></textarea></td>
            </tr>
            <tr>
